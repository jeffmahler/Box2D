...
void b2Separator::Separate(b2Body* pBody, b2FixtureDef* pFixtureDef, vector<b2Vec2>* pVerticesVec, int scale) {
    ...
    for (i=0; i<n; i++) {
        ...
        m = vec.size();        
        if (m > 8) {
          std::cout << "Illegal polygon" << std::endl;
        }
...
void b2Separator::ConvexDecomp(vector<b2Vec2> &pVerticesVec, vector<vector<b2Vec2> > &result) {
    vector<b2Vec2> vec;
    int i, n, j,minLen;
    float d, t, dx, dy;
    int i1, i2, i3;
	...
	// std::cout << "Front size " << queue.front().size() << std::endl;
	// std::cout << "N " << n << std::endl;
	// std::cout << "ISCONVEX " << isConvex << std::endl;
        if (n > 8 && isConvex) {
	  // std::cout << "HERE" << std::endl;
          int size1 = (int)(n / 2);
          vector<b2Vec2> conv_vec1;
          vector<b2Vec2> conv_vec2;
          for (unsigned int z = 0; z < n; z++) {
            if (z == size1 - 1) {
              conv_vec2.push_back(vec[z]);
            }

            if (z < size1) {
              conv_vec1.push_back(vec[z]);
            }
            else {
              conv_vec2.push_back(vec[z]);
            }
          }
          conv_vec2.push_back(vec[0]);

          queue.push(conv_vec2);
          queue.push(conv_vec1);
          queue.pop();
        }
        else if (isConvex) { // && queue.front().size() <= 8) {
